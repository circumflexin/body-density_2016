 function       [MagAcc,pry,Sa,GL,KK] = magnet_rot_sa_AB(Aw,Mw,fs,cutoff,alpha,n,k,J,tmax,subdetections)
...
%           generated.
%       subdetections = run subdetections for one-way spikes in PRY which exceed J. Boolean, defaults to true.
%
%
%    OUTPUT:
...
glk = [K(k,1)-1 K(k+1,2)+1] ; % sampling indexes of glide start stop

for k=1:length(glk), % for each glide
    kk = glk(k,1):glk(k,2) ; % select entire glide
    test=find (isnan(pry(kk,n))); % are any of the PRY values NaN?
    if ~isempty(test), % if so, discard this glide
        glc(k)=NaN;
        glk(k,1)=NaN;
        glk(k,2)=NaN;
    end
...
glk = glk(~isnan(glk(:,1)),:);% drop the glides where pry == NaN

 if ~exist('subdetections','var')
     % defaualt to true if not explicitly passed.
      subdetections = true;
 end

% AB - run optional subdetection inside long glides for one way spikes
% which exceed J.
if subdetections == true
    glk2 =[]; 
    for k=1:length(glk) % for each glide
        kk = glk(k,1):glk(k,2) ; % all samples in glide
        stable = abs(pry(kk,n))<J;% under or over J
        d = diff(stable); % indices where the stable changes 
        starts = find(d > 0)+1; % start of stable sections
        stops = find(d < 0)+1; % end of stable sections
        if stable(1)==1
            starts = [1;starts];
        end
        if length(starts)>length(stops) % catch cases where the glide ends before the stable section stops.
            stops(length(starts))=length(stable)+1;
            stops = reshape( stops, [],1 );
        end
        durs = stops-starts; % durations
        subglides = [glk(k,1)+starts, glk(k,1)+stops];
        %subglides = subglides(durs>tmax*fs/2,:);
        glk2 = [glk2;subglides];
        glk1 = glk(k,1);
    end
    glk = glk2; % overwrite first round detections with subdetections. 
end
