...
help finddives
%define mindivedef as the minimum depth at which to recognize a dive.
mindivedef = 5; %
T=finddives(p,fs,mindivedef,[],1); 
...
[S,f]=speclev(Aw(DES,:),512,fs);plot(f,S);
[S,f]=speclev(Aw(ASC,:),512,fs);plot(f,S);
%here the power spectra is calculated of the longitudinal and 
...
FR = mean(FR(7:8)); % or set based on prior graph
FR= 4.2
f = mean(FRl(7:8))/FR; % from last graph
...
% for dive=1:nnn; %1:nnn% ES - Line added so as to only include until dive where clicked previously on dive profile    
%   
%     
%     kk=round(fs*T(dive,1)):round(fs*T(dive,2)); % it is selecting the whole dive
%     kkI = false(size(p)); kkI(kk) = true;
%     enddes=round(find((smoothpitch(kkI&p>mindivedef*.75)*180/pi)>0,1,'first')+T(dive,1)*fs)-15*fs;% search for the first point after diving below mindivedef at which pitch is positive
%     startasc=round(find((smoothpitch(kkI&p>mindivedef*.75)*180/pi)<0,1,'last')+T(dive,1)*fs)+15*fs;%search for the last point beroe diving above mindivedef at which the pitch is negative
...
%     %%%%%edited by Eilidh, cheked if it works!
...
for dive=1:nnn
    kk=round(fs*T(dive,1)):round(fs*T(dive,2)); % it is selecting the whole dive
    enddes=((find((smoothpitch(kk)*180/pi)>0,1,'first')+T(dive,1)*fs)-15*fs);% search for the first point at which pitch is positive
    startasc=((find((smoothpitch(kk)*180/pi)<0,1,'last')+T(dive,1)*fs)+15*fs);%search for the last point at which the pitch is negative
...
%
speedr=interp(SP5,32);
hold on
speed=speedr(1:size(Aw,1));
plot((1:length(p))/fs,speed)
...
DTS = CTD;  % ES - insert correct matrix name; make sure downcast only
...
%tmax is the maximum duration allowable for a fluke stroke in seconds, it
%can be set as 1/FR%
...
maxy = max(max(Ahf(round(T(1,1)*fs):round(T(nnn,2)*fs),[1 3])));
...
flukeAcc3D = Ahf(DES,3); % hpf-z acc  desscent
...
%7.2_using the body rotations (pry) estimated using the
...
maxy = max(pry(round(T(1,1)*fs):round(T(nnn,2)*fs)));
...
set(ax2(2),'ylim',[min(Ahf([ASC DES])) max(Ahf([ASC DES]))]);
...
%%8.5_AFTER A 5 SEC SUB-GLIDE REMOVE THE NEXT 5S SUB-GLIDE TO AVOID
%%AUTOCORRELATION. 

%%

%%
%9_create summary table required for body density 
